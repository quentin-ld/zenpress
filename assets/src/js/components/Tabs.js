import { useState, createContext, useContext, useRef, useEffect } from '@wordpress/element';
...
export const Tabs = ({ selectedTabId: controlledId, onSelect, orientation = 'horizontal', children }) => {
    ...
    return (
        <TabsContext.Provider
            value={{ selectedTabId: selectedId, onSelect: handleSelect, orientation, getOrderedTabIds, tabListRef }}
        >
...
export const Tab = ({ tabId, title, className = '', children }) => {
    const { selectedTabId, onSelect, orientation, getOrderedTabIds } = useContext(TabsContext);
    ...
    const handleKeyDown = (e) => {
        ...
        if (targetIndex !== currentIndex && targetIndex >= 0 && targetIndex < tabIds.length) {
            const targetTabId = tabIds[targetIndex];
            const targetTabElement = document.getElementById(`zenpress-tab-${targetTabId}`);
            if (targetTabElement) {
